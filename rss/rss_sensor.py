...
class RssSensor(object):
    ...
    def __init__(self, parent_actor, world, unstructured_scene_visualizer, bounding_box_visualizer, state_visualizer, routing_targets=None):
        ...
        self.sensor:carla.RssSensor = world.spawn_actor(bp, carla.Transform(carla.Location(x=0.0, z=0.0)), attach_to=self._parent)
        ...
        """
        RSS situation이 계산을 수행하기 전 호출되는 callback을 등록할 수 있는 함수
        callback에는 carla.RssActorConstellationResult가 전달됨
        """
    ...
    def get_default_parameters():
        """Return a default setting RssDynamics object

        - RSS 값을 계산하기 위한 파라미터를 설정할 수 있는 객체 (RSS 파라미터는 아래 링크 참조)
          @ https://intel.github.io/ad-rss-lib/ad_rss/Appendix-ParameterDiscussion/

        - vehicle, pedestrian 등에 설정
          @ https://carla.readthedocs.io/en/0.9.10/python_api/#carla.RssSensor
        """
